...
include "m8c.inc"
include "memory.inc"
include "RECEIVE_2.inc"


;-----------------------------------------------
;  Global Symbols
;-----------------------------------------------
export   RECEIVE_2_EnableInt
export  _RECEIVE_2_EnableInt
export   RECEIVE_2_DisableInt
export  _RECEIVE_2_DisableInt
export   RECEIVE_2_Start
export  _RECEIVE_2_Start
export   RECEIVE_2_Stop
export  _RECEIVE_2_Stop
export   RECEIVE_2_bReadRxData
export  _RECEIVE_2_bReadRxData
export   RECEIVE_2_bReadRxStatus
export  _RECEIVE_2_bReadRxStatus

; Old function name convension, do not use.
; These will be removed in a future release.
export  bRECEIVE_2_ReadRxData
export _bRECEIVE_2_ReadRxData
export  bRECEIVE_2_ReadRxStatus
export _bRECEIVE_2_ReadRxStatus

;-----------------------------------------------
; High Level RX functions
;-----------------------------------------------

export  RECEIVE_2_cGetChar
export _RECEIVE_2_cGetChar
export  RECEIVE_2_cReadChar
export _RECEIVE_2_cReadChar
export  RECEIVE_2_iReadChar
export _RECEIVE_2_iReadChar

IF (RECEIVE_2_RXBUF_ENABLE)
export  RECEIVE_2_CmdReset
export _RECEIVE_2_CmdReset
export  RECEIVE_2_bCmdCheck
export _RECEIVE_2_bCmdCheck
export  RECEIVE_2_bCmdLength
export _RECEIVE_2_bCmdLength
export  RECEIVE_2_bErrCheck
export _RECEIVE_2_bErrCheck

export  RECEIVE_2_szGetParam
export _RECEIVE_2_szGetParam
export  RECEIVE_2_szGetRestOfParams
export _RECEIVE_2_szGetRestOfParams

;-----------------------------------------------
;  Variables
;-----------------------------------------------

AREA RECEIVE_2_RAM(RAM,REL,CON)
 ptrParam:   			BLK  1
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_EnableInt
;
;  DESCRIPTION:
...
;
 RECEIVE_2_EnableInt:
_RECEIVE_2_EnableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_EnableIntMask RECEIVE_2_INT_REG, RECEIVE_2_bINT_MASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_DisableInt
;
;  DESCRIPTION:
...
;    functions.
;
 RECEIVE_2_DisableInt:
_RECEIVE_2_DisableInt:
   RAM_PROLOGUE RAM_USE_CLASS_1
   M8C_DisableIntMask RECEIVE_2_INT_REG, RECEIVE_2_bINT_MASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_Start(BYTE bParity)
;
;  DESCRIPTION:
...
_RECEIVE_2_Start:
   ...
   mov   REG[RECEIVE_2_CONTROL_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
_RECEIVE_2_Stop:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   REG[RECEIVE_2_CONTROL_REG], ~bfCONTROL_REG_START_BIT
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_bReadRxData
;
;  DESCRIPTION:
...
;    functions.
;
 RECEIVE_2_bReadRxData:
_RECEIVE_2_bReadRxData:
 bRECEIVE_2_ReadRxData:
_bRECEIVE_2_ReadRxData:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A, REG[RECEIVE_2_RX_BUFFER_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_bReadRxStatus
;
;  DESCRIPTION:
...
;    functions.
;
 RECEIVE_2_bReadRxStatus:
_RECEIVE_2_bReadRxStatus:
 bRECEIVE_2_ReadRxStatus:
_bRECEIVE_2_ReadRxStatus:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov A,  REG[RECEIVE_2_CONTROL_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_cGetChar
;
;  DESCRIPTION:
...
.getChar_Loop:
   tst REG[RECEIVE_2_CONTROL_REG],RECEIVE_2_RX_REG_FULL   ; Check if a character is ready
   jz  .getChar_Loop                                        ; If not loop

   mov A, REG[RECEIVE_2_RX_BUFFER_REG]           ; Get character
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_cReadChar
;
;  DESCRIPTION:
;     Read character from UART RX port.
;
...
;
 RECEIVE_2_cReadChar:
_RECEIVE_2_cReadChar:
   RAM_PROLOGUE RAM_USE_CLASS_1

   mov  A,REG[RECEIVE_2_CONTROL_REG]                       ; Get Status of RX
   push A
   and  A,RECEIVE_2_RX_COMPLETE                            ; Check if a character is ready
   jnz  .RX_DATA_RDY                                       ; Data Ready go read it.
   pop  A
   jmp  .RX_NO_VALID_CHAR

.RX_DATA_RDY:
   mov  A,REG[RECEIVE_2_RX_BUFFER_REG]                     ; Read data first, then
   swap A,X                                                ; determine if data is valid

   pop  A                                                  ; Check for errors
   and  A,(RECEIVE_2_RX_PARITY_ERROR | RECEIVE_2_RX_FRAMING_ERROR)
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_iReadChar
;
...
;
 RECEIVE_2_iReadChar:
_RECEIVE_2_iReadChar:
   RAM_PROLOGUE RAM_USE_CLASS_1

   mov  A,REG[RECEIVE_2_CONTROL_REG]                       ; Get Status of RX
                                                           ; Mask only errors and data ready
   and  A,(RECEIVE_2_RX_ERROR|RECEIVE_2_RX_REG_FULL)
   push A
   and  A,RECEIVE_2_RX_COMPLETE                            ; Check if a character is ready
   jnz  .RX_GET_DATA                                       ; Data Ready go read it.
   pop  A
   or   A,RECEIVE_2_RX_NO_DATA                             ; Add no data flag
   swap A,X
   jmp  End_RECEIVE_2_iReadChar

.RX_GET_DATA:
   pop  A
   and  A,RECEIVE_2_RX_ERROR
   swap A,X
   mov  A,REG[RECEIVE_2_RX_BUFFER_REG]                     ; Read data first, then
                                                           ; determine if data is valid

 End_RECEIVE_2_iReadChar:
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_CmdReset
;
;  DESCRIPTION:
...
_RECEIVE_2_CmdReset:
   ...
   RAM_SETPAGE_CUR >RECEIVE_2_aRxBuffer
   mov [RECEIVE_2_aRxBuffer], 0x00
   RAM_SETPAGE_CUR >ptrParam
   mov [ptrParam],0x00
   RAM_SETPAGE_CUR >RECEIVE_2_bRxCnt
   mov [RECEIVE_2_bRxCnt], 0x00
   and [RECEIVE_2_fStatus], 0x00
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_bCmdCheck
;
;  DESCRIPTION:
...
;
 RECEIVE_2_bCmdCheck:
_RECEIVE_2_bCmdCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >RECEIVE_2_fStatus
   mov A,  [RECEIVE_2_fStatus]
   and A, RECEIVE_2_RX_BUF_CMDTERM               ; Mask off Command status
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_bErrCheck
;
;  DESCRIPTION:
...
;
 RECEIVE_2_bErrCheck:
_RECEIVE_2_bErrCheck:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >RECEIVE_2_fStatus
   mov A,  [RECEIVE_2_fStatus]
   and A, RECEIVE_2_RX_BUF_ERROR                 ; Mask off Error status
   and [RECEIVE_2_fStatus], ~RECEIVE_2_RX_BUF_ERROR
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_bCmdLength
;
;  DESCRIPTION:
...
;          CUR_PP
;
 RECEIVE_2_bCmdLength:
_RECEIVE_2_bCmdLength:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >RECEIVE_2_bRxCnt
   mov A,  [RECEIVE_2_bRxCnt]
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_szGetParam
;
;  DESCRIPTION:
...
;     RECEIVE_2_CmdReset which resets the entire buffer to the 'empty'
;     state. Typically this function, RECEIVE_2_szGetParam, is
;     not called until the buffer has been loaded with an entire command
;     (See RECEIVE_2_bCmdCheck).
;
...
;
 RECEIVE_2_szGetParam:
_RECEIVE_2_szGetParam:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_PROLOGUE RAM_USE_CLASS_3
   RAM_SETPAGE_CUR >ptrParam
   RAM_SETPAGE_IDX >RECEIVE_2_aRxBuffer

   mov  A, <RECEIVE_2_aRxBuffer             ; Get address to receive buffer
...
.CheckForDelim:
    cmp  A,RECEIVE_2_DELIMITER              ; Check if we have a delimiter
    jnz  .ParamStartFound
    inc  X                                  ; Increment both current pointer and
    inc  [ptrParam]                         ; stored pointer.
    mov  A,[X]                              ; Get character pointed by X
    cmp  [ptrParam],(RECEIVE_2_RX_BUFFER_SIZE -1)  ; Check if we are at the end of buffer
    jnz  .CheckForDelim
                                            ; End of string found
.EndOfString:
    push X                                  ; Save ptr
...
.ParamLoop:
    ...
    mov  [ptrParam],(RECEIVE_2_RX_BUFFER_SIZE -1)  ; Reset pointer to end of string.
...
.NotNullString:
   pop  X
   mov  A,>RECEIVE_2_aRxBuffer                     ; Return pointer
   RAM_EPILOGUE RAM_USE_CLASS_3
   RAM_EPILOGUE RAM_USE_CLASS_4
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: RECEIVE_2_szGetRestOfParams
;
;  DESCRIPTION:
...
;          CUR_PP
;
 RECEIVE_2_szGetRestOfParams:
_RECEIVE_2_szGetRestOfParams:
   RAM_PROLOGUE RAM_USE_CLASS_4
   RAM_SETPAGE_CUR >ptrParam
